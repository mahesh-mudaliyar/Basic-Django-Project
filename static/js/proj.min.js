...
function addrow() {
  var trLength=$("#tblData").find('tr').length;
  var i= 1;
  $("#tblData").find('tr').each(function () {
    var trow = $(this);
    if ($("#tblData").find('td').length === 0) {
      trow.append('<td onclick="togglecls(this);"><input style="width:100%;" type="text" /> </td><td><input class="rows-delete btn btn-danger" type="button" value="Delete"  style="width:100%;"></td>');
    } else {
      trow.prepend('<td onclick="togglecls(this);"><input style="width:100%;" type="text" /></td>');
    }
    i++;
  });
  if($("#tblData").find('tr').length>1){
    $("#tblData").find('tr:last td').html('<input class="cols-delete btn btn-danger" type="button" value="Delete"  style="width:100%;">')
  }
  $("#saveTbl").show();
  $("#delTbl").show();
  $("#mergeTbl").show();
...
function addcol() {
  var markup = "", delup = "";
  tdLength = $("#tblData").find('tr:first td').length
  for (var i = 1; i <= tdLength; i++) {
    if (tdLength == i) {
      markup += '<td><input class="rows-delete btn btn-danger" type="button" value="Delete"  style="width:100%;"</td>';
    } else {
      delup += '<td><input class="cols-delete btn btn-danger" type="button" value="Delete"  style="width:100%;"></td>'
      markup += "<td onclick='togglecls(this);'><input style='width:100%;' type='text' /></td>";
    }
  }
  if ($("#tblData").find('tr').length == 1) {
    $("#tblData").append("<tr>" + delup + "</tr>");
  }
  $("#tblData").prepend("<tr>" + markup + "</tr>");
  markup = "";
  $("#saveTbl").show();
  $("#delTbl").show();
  $("#mergeTbl").show();
...
$("table").on("click", ".cols-delete", function ( event ) {
...
$("table").on("click", ".rows-delete", function ( event ) {
  if($("#tblData").find("tr:first td").length>2){
    swal({
      title: "Are you sure?",
      text: "Once Deleted, you cannot undo!",
